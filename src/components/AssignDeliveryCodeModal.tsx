...
export default function AssignDeliveryCodeModal({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      ...
      >
        <h2 className="text-2xl font-bold mb-4 text-gray-800">Assign Delivery Code</h2>
        <p className="text-sm text-gray-600 mb-6">
          Select a delivery code for this delivery boy. Only codes that are not already assigned are shown.
        </p>
            ...
            {deliveryCodes.map((dc) => (
              <label
                ...
                className={`flex items-start space-x-3 p-3 rounded-lg cursor-pointer transition-colors ${
                  selectedId === dc.id ? "bg-blue-50 border border-blue-200" : "hover:bg-gray-50"
                }`}
              >
                <input
                  type="radio"
